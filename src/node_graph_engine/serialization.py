...
class AiidaSerializationAdapter(SerializationAdapter):
    ...
    def __init__(
        self, serializers: Optional[Dict[str, str]] = None, user: Any = None
    ) -> None:
    ...
    def serialize(self, value: Any, socket: Any, *, store: bool) -> Any:
        if socket is None:
            return value
        resolved = resolve_tagged_values(value)
        return serialize_ports(
            python_data=resolved,
            port_schema=socket._to_spec(),
    ...
    def serialize_ports(
        ...
        resolved = resolve_tagged_values(python_data)
        return serialize_ports(
            python_data=resolved,
